...
func (s *InstanceStop) Initialize(attrs map[string]interface{}) pluginx.EndpointCallable {
	//  初始化context
	...
	return &InstanceStop{attributes: attrs, ctx: context}
}

func (s *InstanceStop) Run(input InstanceStopInput) (interface{}, error) {
	log.Debugf(s.ctx, "Instance Stop got input [%v]. The correlation information [%v] ", input, s.attributes)
	...
	if tmp, ok := input.Body["force_stop"]; ok {
		if tmp == "true" {
			input.Body["force_stop"] = true
		} else {
			input.Body["force_stop"] = false
		}
	}
	body_json, _ := json.Marshal(map[string]interface{}{
		"os-stop": input.Body,
	})
