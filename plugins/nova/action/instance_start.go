...
func (s *InstanceStart) Initialize(attrs map[string]interface{}) pluginx.EndpointCallable {
	//  初始化context
	...
	return &InstanceStart{attributes: attrs, ctx: context}
}

func (s *InstanceStart) Run(input InstanceStartInput) (interface{}, error) {
	log.Debugf(s.ctx, "Instance Start got input [%v]. The correlation information [%v] ", input, s.attributes)
	...
	body_json, _ := json.Marshal(map[string]interface{}{"os-start": input.Body})
